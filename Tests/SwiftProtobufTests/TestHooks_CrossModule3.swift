...
extension CrossModule3_Msg {
  var hashValue: Int {
    print("CrossModule3_Msg.hashValue called!")
    return field1.hashValue
  }

  func isEqualTo(message: CrossModule3_Msg) -> Bool {
    print("CrossModule3_Msg.isEqualTo(message:) called!")
    return field1 == message.field1
  }
}


class TestHooks_CrossModule3: XCTestCase {
  // Type aliases so diffing test files is easier.
  typealias MESSAGE = CrossModule3_Msg
  typealias MESSAGEAsField = CrossModule3_MsgAsField
  typealias MESSAGEInArray = CrossModule3_MsgInArray
  typealias MESSAGEInArrayInHeapStorage = CrossModule3_MsgInArrayInHeapStorage
  typealias MESSAGEInMap = CrossModule3_MsgInMap
  typealias MESSAGEInMapInHeapStorage = CrossModule3_MsgInMapInHeapStorage
