...
extension SameModule2_HeapStorage_Msg {
  var hashValue: Int {
    print("SameModule2_HeapStorage_Msg.hashValue called!")
    return field1.hashValue
  }

  func isEqualTo(message: SameModule2_HeapStorage_Msg) -> Bool {
    print("SameModule2_HeapStorage_Msg.isEqualTo(message:) called!")
    return hasField1 == message.hasField1 && field1 == message.field1
  }
}


class TestHooks_SameModule2_HeapStorage: XCTestCase {
  // Type aliases so diffing test files is easier.
  typealias MESSAGE = SameModule2_HeapStorage_Msg
  typealias MESSAGEAsField = SameModule2_HeapStorage_MsgAsField
  typealias MESSAGEInArray = SameModule2_HeapStorage_MsgInArray
  typealias MESSAGEInArrayInHeapStorage = SameModule2_HeapStorage_MsgInArrayInHeapStorage
  typealias MESSAGEInMap = SameModule2_HeapStorage_MsgInMap
  typealias MESSAGEInMapInHeapStorage = SameModule2_HeapStorage_MsgInMapInHeapStorage
